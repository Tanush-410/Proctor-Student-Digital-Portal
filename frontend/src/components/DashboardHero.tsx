...
export function DashboardHero({ eyebrow, title, subtitle, action }: { eyebrow?: string; title: string; subtitle?: string; action?: ReactNode }) {
  return (
    <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-brand-700 via-brand-600 to-brand-900 px-6 py-7 shadow-card sm:px-8 sm:py-9">
      ...
      {/* Framer Motion owns the `transform` CSS property outright once
          `initial`/`animate` touch it (scale/rotate here) — a Tailwind
          -translate-x-1/2/-translate-y-1/2 class on the same element gets
          silently overwritten by Framer's own computed transform, so the
          centering offset has to be expressed as x/y motion values instead
          of a class, or it never actually applies. */}
      <motion.img
        src="/bms-logo.svg"
        alt=""
        aria-hidden="true"
        initial={{ opacity: 0, scale: 0.9, rotate: -6, x: "-50%", y: "-50%" }}
        animate={{ opacity: 0.14, scale: 1, rotate: 0, x: "-50%", y: "-50%" }}
        transition={{ duration: 0.9, ease: [0.22, 1, 0.36, 1] }}
        className="pointer-events-none absolute left-1/2 top-1/2 h-56 w-56 sm:h-64 sm:w-64"
      />
      <motion.div
